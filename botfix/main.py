...
async def safe_send_and_track(client, chat_id, text, reply_to=None, delay=0.0,
    ...
    try:
        ...
        for _attempt in range(3):   # auto-retry up to 3× on FloodWait (SAFE_MODE)
            try:
                ...
                # Auto-handle flood waits when SAFE_MODE is ON
                if cfg.get("SAFE_MODE", False) and fw.seconds <= 120:
                    ...
                    continue   # retry
                return None    # safe mode off or too long — give up
        else:
...
async def main():
    ...
    # ── Reconnect loop ────────────────────────────────────────────────────────
    # Instead of returning when clients disconnect (which exits the process and
    # forces the platform to cold-restart every ~1 minute), we stay inside this
    # loop: when all clients drop, wait 15 s, reconnect, and keep running.
    # This eliminates the crash-restart cycle that was causing SYSTEM_BOOT every
    # 1–10 minutes.
